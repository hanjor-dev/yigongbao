...
- 机构名称为 `null`、空串或纯空白时仍保留对象；`null` 统一转换为空字符串，空串或纯空白保留原值。这样每个对象的 JSON 都稳定包含 `orgName`，同时名称完整性问题不会导致合法机构权限从详情中消失。
- 无额外管理机构时返回 `[]`。
...
- `orgName` 为 `null` 时保留对象并返回空字符串；空串或纯空白时保留对象和原值，控制器级序列化断言每个对象均包含 `orgName`。
- 主机构已删除、停用或类型错误但额外机构有效时，断言 `managedOrgIds`、`managedOrgs` 仍回显有效额外机构，而 `effectiveOrgIds=[]`。
