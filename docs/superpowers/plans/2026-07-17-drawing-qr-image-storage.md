...
- [ ] **Step 3: Implement current-file lookup and snapshot**
  ...
  Resolve the newest associated file from `fileService.listByBiz`, download it with `downloadToBytes`, set `ctx.qrBytes`, and persist `qrFileId` alongside the generated drawing file. Make `ensureDrawing` compare the current QR file ID with the latest AUTO drawing snapshot: a changed QR invalidates/rebuilds an AUTO drawing, while a MANUAL drawing remains the source of truth and is not mutated or regenerated solely because the QR changed. A historical AUTO drawing with null `qr_file_id` is treated as stale when a current QR exists. If an existing drawing can be reused, a missing current QR does not invalidate that historical file; if data or QR changes require a new AUTO generation and no current QR exists, return a clear business error. A new order/package with no drawing and no current QR also fails clearly. Avoid calling the old URL builder or backend QR generator.
