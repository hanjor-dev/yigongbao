...
public interface RebuildProjectService extends IService<RebuildProjectEntity> {
     ...
     * @return 项目树形列表
     */
    List<RebuildProjectVO> listTree(String categoryCode, String keyword);
     ...
     * @return 该部位下的项目树
     */
    List<RebuildProjectVO> listByBodyPartId(Long bodyPartId, String categoryCode, String keyword);
     ...
     * 仅包含启用状态的部位和项目；有 keyword 时只返回名称匹配的项目及其父项目，无匹配项目的部位不出现
     *
     ...
     * @return 按部位分组的项目树列表
     */
    List<BodyPartProjectTreeVO> listFullTree(String categoryCode, Long bodyPartId, String keyword);
