...
INSERT INTO sys_config (config_key, config_name, config_value, config_type, config_group, config_desc, is_system, is_public, sort, status)
VALUES ('production.process.params.config', '工序参数配置字典', '{"print":{"layerThickness":{"label":"层厚","type":"number","unit":"mm","required":true},"laserPower":{"label":"激光器功率","type":"number","unit":"mW","required":true}},"wash":{"alcoholBatchNo":{"label":"酒精批号","type":"text","required":true},"soakLevel":{"label":"浸泡程度","type":"select","options":["完全浸泡","部分浸泡"],"required":true},"startHour":{"label":"浸泡开始时间（时）","type":"number","required":true},"startMinute":{"label":"浸泡开始时间（分）","type":"number","required":true}},"cure":{"cureMode":{"label":"固化模式","type":"select","options":["HIGH","LOW"],"required":true},"startHour":{"label":"固化开始时间（时）","type":"number","required":true},"startMinute":{"label":"固化开始时间（分）","type":"number","required":true},"endHour":{"label":"固化结束时间（时）","type":"number","required":true},"endMinute":{"label":"固化结束时间（分）","type":"number","required":true}},"clean_dry":{"alcoholBatchNo":{"label":"酒精批号","type":"text","required":true},"cleanMode":{"label":"清洗模式","type":"select","options":["变波","脱气"],"required":true},"heating":{"label":"加热","type":"select","options":["开","关"],"required":true},"startHour":{"label":"清洗开始时间（时）","type":"number","required":true},"startMinute":{"label":"清洗开始时间（分）","type":"number","required":true},"endHour":{"label":"清洗结束时间（时）","type":"number","required":true},"endMinute":{"label":"清洗结束时间（分）","type":"number","required":true}},"pack":{"h2o2Sterilization":{"label":"过氧化氢等离子灭菌","type":"switch","required":false},"sealTemperature":{"label":"指示包装袋热封温度","type":"number","unit":"℃","required":true},"sealTime":{"label":"热封时间","type":"number","unit":"秒","required":true},"zipBagSeal":{"label":"三边封拉链袋热封","type":"switch","required":false,"dependents":{"zipBagSealTemperature":{"label":"三边封拉链袋热封温度","type":"number","unit":"℃","required":true},"zipBagSealTime":{"label":"热封时间","type":"number","unit":"秒","required":true}}}}}', 'json', 'production', '各工序参数字段定义（JSON），前端据此动态渲染参数表单', 1, 0, 3, 1);
...
(1204, 102, '填写打印信息',    'design:PrintInfo',        3, 4,  1),
(1205, 102, '图纸指令单预览',  'design:DocumentPreview',  3, 5,  1),
(1206, 102, '影像在线预览',    'design:StlView',          3, 6,  1),
(1207, 102, '生成二维码',      'design:QrCode',           3, 7,  1),
...
-- 质检线角色（企业内部）
(8,  '质管',         'qc-inspector',        '质检执行，可见全部数据',                                       '6.1', 'all',       1),
-- 仓储线角色（企业内部）
...
(11, 1018), (11, 1019), (11, 1020), (11, 1021),
