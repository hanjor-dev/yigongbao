...
-- 设计文件上传相关配置
INSERT INTO sys_config (config_key, config_name, config_value, config_type, config_group, config_desc, is_system, is_public, sort, status) VALUES
('design.package.allowed_extensions', '打印文件数据包允许的文件类型', '.zip,.rar,.7z', 'string', 'design', '数据包压缩包内允许的文件扩展名，逗号分隔', 0, 0, 101, 1),
('design.package.max_size_mb', '打印文件数据包最大大小', '500', 'number', 'design', '数据包文件最大大小（MB）', 0, 0, 102, 1),
...
('design.report.allowed_extensions', '设计报告允许的文件扩展名', '.pdf,.doc,.docx,.xls,.xlsx', 'string', 'design', '设计报告允许的文件扩展名，逗号分隔（word/pdf/excel）', 0, 0, 105, 1),
